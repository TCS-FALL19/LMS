...
router.put('/classes/:cid', (req, res, next) => {
  Class.findOneAndUpdate({ _id: req.params.tid }, req.body, {new: true}, function(error, results) {
    if (error) {
        return next(error);
    }
    res.json(results);
...
router.put('/teachers/:tid', (req, res, next) => {
  Teacher.findOneAndUpdate({ _id: req.params.tid }, req.body, {new: true}, function(error, results) {
    if (error) {
        return next(error);
    }
    res.json(results);
